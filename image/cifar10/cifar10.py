...
# tf.app: generic entry point script
...
def _variable_with_weight_decay(name, shape, stddev, wd):
  ...
  var = _variable_on_cpu(
      ...
      # truncated normal distribution: normal distribution bounded above / below
  if wd is not None:
...
def inference(images):
  ...
  with tf.variable_scope('local3') as scope:
    ...
    # images.get_shape().as_list()[0] = 32, 64x8x8x3/32 = 384
...
def loss(logits, labels):
  ...
  # logits: unscaled log probability of prediction
...
def train(total_loss, global_step):
  ...
  # Compute gradients.
  # this will only run after loss_averages_op is finished
  ...
  # Add histograms for trainable variables (with trainable=True)
